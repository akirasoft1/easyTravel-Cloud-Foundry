...
if ! create-docker-machine "${DOCKER_MACHINE_NAME}" "${DOCKER_MACHINE_AWS_OWNER}" "${DOCKER_MACHINE_AWS_CATEGORY}" "${DOCKER_MACHINE_AWS_REGION}" "${DOCKER_MACHINE_AWS_INSTANCE_TYPE}" "${DOCKER_MACHINE_TMP_FILES}" "${DOCKER_MACHINE_AWS_NAME_FILE}"; then
